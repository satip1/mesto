...
function handleDeleteCard(obj) {
  popupsObject.delCard.setDelCard(obj)
  popupsObject.delCard.open();
}

// обработчик события submit формы подтверждения удаления карточки
function handleSubmitDelCard(evt, obj) {
  evt.preventDefault();
  const elem = obj.getDelCard();
  const id = elem.getCardIdPicture();
  api.deleteCurrentCard(id)
    .then(res => elem.deleteCard())
...
function handleClickProfileBtnEdit() {
  ...
  validClass.editprofile__form.setDisabledBtnSave();
  popupsObject.editprofile.open();
}

// обработчик события submit формы редактирования .popup при нажатии кнопки Сохранить
function handleSubmitEditProfileForm(event, obj) {
  event.preventDefault();
  obj.setTextBtnSave(true);
  const { edit__name: dataUser, edit__about: dataJob } = obj.getInputValues();
...
function handleClickProfileBtnAdd() {
  validClass.addcard__form.setDisabledBtnSave();
  popupsObject.addcard.open();
}

// обработчик события submit формы добавления карточки .addcard при нажатии кнопки Сохранить
function handleSubmitAddcardForm(event, obj) {
  event.preventDefault();
  obj.setTextBtnSave(true);
  const idUserFoto = userAbout.getUserId();
  const { addcard__name: dataText, addcard__about: dataLink } = obj.getInputValues();
...
function handleClickAvatarBtnEdit() {
  validClass.editavatar__form.setDisabledBtnSave();
...
function handleSubmitEditAvatar(event, obj) {
  event.preventDefault();
  const { editavatar__about: dataSrc } = obj.getInputValues();
  obj.setTextBtnSave(true);
